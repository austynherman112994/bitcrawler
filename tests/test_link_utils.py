...
def test_get_domain():
    ...
    assert link_utils.get_domain(url1) == "python.org"
    assert link_utils.get_domain(url1) == "python.org"
    assert link_utils.get_domain(url2) == "python.org"

def test_is_same_domain():
    url1 = "http://python.org/test/1?some_param=some_val"
    url2 = "http://python.org/search/123"
    url3 = "http://www.test.python.org:8080"
    assert link_utils.is_same_domain(url1, url2)
    assert link_utils.is_same_domain(url1, url3)
    assert link_utils.is_same_domain(url2, url3)

def test_not_is_same_domain():
    url1 = "http://python.org/test/1?some_param=some_val"
    url2 = "http://pandas.org/search/123"
    assert not link_utils.is_same_domain(url1, url2)
